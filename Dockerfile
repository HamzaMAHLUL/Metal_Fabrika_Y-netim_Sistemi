...
WORKDIR /src
COPY ["Metal_Fabrika_Yönetim_Sistemi.csproj", "."]
RUN dotnet restore "./Metal_Fabrika_Yönetim_Sistemi.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./Metal_Fabrika_Yönetim_Sistemi.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Metal_Fabrika_Yönetim_Sistemi.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "Metal_Fabrika_Yönetim_Sistemi.dll"]